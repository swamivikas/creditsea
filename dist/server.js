...
app.get('/users/:id/loan_requests', async (req, res) => {
...
app.post('/users/:id/loan_requests', async (req, res) => {
    const userId = parseInt(req.params.id);
    ...
    try {
        ...
        if (userResult.rows.length === 0) {
            await db.query('INSERT INTO users (user_id, name) VALUES ($1, $2)', [userId, name]);
        }
        const loanResult = await db.query(`INSERT INTO loan_requests (user_id, loan_amount, tenure, employment_status, employment_address, loan_reason) 
             VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`, [userId, loan_amount, tenure, employment_status, employment_address, loan_reason]);
...
app.get('/loan_requests', async (req, res) => {
    try {
...
app.put('/loan_requests/:loan_id', async (req, res) => {
    const loanId = parseInt(req.params.loan_id);
    const { status } = req.body;
    try {
        const result = await db.query('UPDATE loan_requests SET status = $1, updated_at = CURRENT_TIMESTAMP WHERE loan_id = $2 RETURNING *', [status, loanId]);
...
app.get('/analytics/loans', async (req, res) => {
    try {
